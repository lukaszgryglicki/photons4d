package photons4d

import (
	"math"
	"testing"
)

// tiny helper
func bumpPoint(p Point4, d Vector4, eps Real) Point4 {
	return Point4{p.X + d.X*eps, p.Y + d.Y*eps, p.Z + d.Z*eps, p.W + d.W*eps}
}

func TestRefractionThrough_Cell8(t *testing.T) {
	ior := Real(1.1) // mild to avoid TIR in weird angles
	h, err := NewCell8(
		Point4{0, 0, 0, 0.6},
		Vector4{0.3, 0.3, 0.3, 0.3},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1} // shoot along +W

	// 1) enter
	h1, ok := intersectRayCell8(O, D, h)
	if !ok || h1.inv {
		t.Fatalf("expected entering hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))

	T1, ok := refract4(D, h1.Nw, 1/ior) // air->material
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	// 2) inside -> move a bit and hit far side
	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCell8(O2, T1, h)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	// 3) exit refraction
	T2, ok := refract4(T1, h2.Nw, ior) // material->air
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	// sanity: exiting means outward normal has positive dot with outgoing dir
	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	// and we shouldn't immediately re-enter a convex object
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayCell8(O3, T2, h); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_HyperSphere(t *testing.T) {
	ior := Real(1.1)
	s, err := NewHyperSphere(
		Point4{0, 0, 0, 0.6},
		Vector4{0.25, 0.25, 0.25, 0.25},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayHyperSphere(O, D, s)
	if !ok || h1.inv {
		t.Fatalf("expected entering sphere hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayHyperSphere(O2, T1, s)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit sphere hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayHyperSphere(O3, T2, s); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}

	// axis symmetry sanity: for axis case, final dir should still be "forward"
	if D.Dot(T2) <= 0 {
		t.Fatalf("emerged going backwards: D·T2=%.12g", D.Dot(T2))
	}
}

func TestRefractionThrough_Cell5(t *testing.T) {
	ior := Real(1.1)
	sx, err := NewCell5(
		Point4{0, 0, 0, 0.6},
		Vector4{.3, .3, .3, .3},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayCell5(O, D, sx)
	if !ok || h1.inv {
		t.Fatalf("expected entering 5-cell hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCell5(O2, T1, sx)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit 5-cell hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayCell5(O3, T2, sx); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Cell16(t *testing.T) {
	// Requires NewCell16 + intersectRayCell16 to be implemented.
	ior := Real(1.1)
	c16, err := NewCell16(
		Point4{0, 0, 0, 0.6},
		Vector4{.3, .3, .3, .3},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayCell16(O, D, c16)
	if !ok || h1.inv {
		t.Fatalf("expected entering 16-cell hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCell16(O2, T1, c16)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit 16-cell hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayCell16(O3, T2, c16); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Cell24(t *testing.T) {
	// Requires NewCell24 + intersectRayCell24 to be implemented.
	ior := Real(1.1)
	c24, err := NewCell24(
		Point4{0, 0, 0, 0.6},
		Vector4{.3, .3, .3, .3},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayCell24(O, D, c24)
	if !ok || h1.inv {
		t.Fatalf("expected entering 24-cell hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCell24(O2, T1, c24)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit 24-cell hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayCell24(O3, T2, c24); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

// Optional tiny numeric sanity for unit-length directions
func almostUnit(v Vector4) bool {
	l := v.Len()
	return math.Abs(float64(l-1)) < 1e-9
}

func TestRefractionThrough_Cell120(t *testing.T) {
	ior := Real(1.05)
	obj, err := NewCell120(
		Point4{0, 0, 0, 0.6},
		Vector4{.2, .2, .2, .2},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatalf("NewCell120: %v", err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayCellPoly(O, D, &obj.cellPoly)
	if !ok || h1.inv {
		t.Fatalf("expected entry hit; ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCellPoly(O2, T1, &obj.cellPoly)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hit; ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	// should continue generally forward in +W
	if T2.W <= 0 {
		t.Fatalf("expected +W after exit, got %+v", T2)
	}
	_ = P2
}

func TestRefractionThrough_Cell600(t *testing.T) {
	ior := Real(1.05)
	obj, err := NewCell600(
		Point4{0, 0, 0, 0.6},
		Vector4{.2, .2, .2, .2},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatalf("NewCell600: %v", err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayCellPoly(O, D, &obj.cellPoly)
	if !ok || h1.inv {
		t.Fatalf("expected entry hit; ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayCellPoly(O2, T1, &obj.cellPoly)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hit; ok=%v inv=%v", ok, h2.inv)
	}
	_ = O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()
	if T2.W <= 0 {
		t.Fatalf("expected +W after exit, got %+v", T2)
	}
}

func TestRefractionThrough_Spherinder(t *testing.T) {
	ior := Real(1.1)
	s, err := NewSpherinder(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.2, 0.2, 0.15},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRaySpherinder(O, D, s)
	if !ok || h1.inv {
		t.Fatalf("expected entering spherinder hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRaySpherinder(O2, T1, s)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit spherinder hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRaySpherinder(O3, T2, s); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_HyperCone(t *testing.T) {
	ior := Real(1.1)
	c, err := NewHyperCone(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.2, 0.2, 0.15},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	// Off-axis so we enter through the side and exit through the base cap.
	O := Point4{0.05, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayHyperCone(O, D, c)
	if !ok || h1.inv {
		t.Fatalf("expected entering hypercone hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayHyperCone(O2, T1, c)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hypercone hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayHyperCone(O3, T2, c); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_HyperCapsule(t *testing.T) {
	ior := Real(1.1)
	c, err := NewHyperCapsule(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.2, 0.2, 0.1},
		0.15,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayHyperCapsule(O, D, c)
	if !ok || h1.inv {
		t.Fatalf("expected entering hypercapsule hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayHyperCapsule(O2, T1, c)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hypercapsule hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayHyperCapsule(O3, T2, c); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Spheritorus(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewSpheritorus(
		Point4{0, 0, 0, 0.6},
		Vector4{1, 1, 1, 1},
		0.3,
		0.1,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	// Through the tube cross-section centered at local (x=R, w=0), along +Y.
	O := Point4{0.3, -1, 0, 0.6}
	D := Vector4{0, 1, 0, 0}

	h1, ok := intersectRaySpheritorus(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering spheritorus hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRaySpheritorus(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit spheritorus hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRaySpheritorus(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Duotorus(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewDuotorus(
		Point4{0, 0, 0, 0.6},
		Vector4{1, 1, 1, 1},
		0.3,
		0.2,
		0.08,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0.2, 0.6}
	D := Vector4{1, 0, 0, 0}

	h1, ok := intersectRayDuotorus(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering duotorus hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayDuotorus(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit duotorus hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayDuotorus(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Duocylinder(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewDuocylinder(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.3, 0.25, 0.15},
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayDuocylinder(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering duocylinder hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayDuocylinder(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit duocylinder hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayDuocylinder(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Torisphere(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewTorisphere(
		Point4{0, 0, 0, 0.6},
		Vector4{1, 1, 1, 1},
		0.25,
		0.08,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0.6}
	D := Vector4{1, 0, 0, 0}

	h1, ok := intersectRayTorisphere(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering torisphere hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayTorisphere(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit torisphere hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayTorisphere(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_Superquadric(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewSuperquadric(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.3, 0.25, 0.15},
		4,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRaySuperquadric(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering superquadric hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRaySuperquadric(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit superquadric hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRaySuperquadric(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}

func TestRefractionThrough_HyperFrustum(t *testing.T) {
	ior := Real(1.1)
	obj, err := NewHyperFrustum(
		Point4{0, 0, 0, 0.6},
		Vector4{0.2, 0.2, 0.2, 0.15},
		0.5,
		1.0,
		Rot4{},
		RGB{1, 1, 1}, RGB{0, 0, 0}, RGB{0, 0, 0}, RGB{1, 1, 1}, RGB{ior, ior, ior},
	)
	if err != nil {
		t.Fatal(err)
	}

	// Stable cap-to-cap path.
	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}

	h1, ok := intersectRayHyperFrustum(O, D, obj)
	if !ok || h1.inv {
		t.Fatalf("expected entering hyperfrustum hit, ok=%v inv=%v", ok, h1.inv)
	}
	P1 := O.Add(D.Mul(h1.t))
	T1, ok := refract4(D, h1.Nw, 1/ior)
	if !ok {
		t.Fatalf("unexpected TIR at entry")
	}
	T1 = T1.Norm()

	O2 := bumpPoint(P1, T1, bumpShift)
	h2, ok := intersectRayHyperFrustum(O2, T1, obj)
	if !ok || !h2.inv {
		t.Fatalf("expected inside->exit hyperfrustum hit, ok=%v inv=%v", ok, h2.inv)
	}
	P2 := O2.Add(T1.Mul(h2.t))

	T2, ok := refract4(T1, h2.Nw, ior)
	if !ok {
		t.Fatalf("unexpected TIR at exit")
	}
	T2 = T2.Norm()

	if h2.Nw.Dot(T2) <= 1e-9 {
		t.Fatalf("not exiting outward: N·T2=%.12g", h2.Nw.Dot(T2))
	}
	O3 := bumpPoint(P2, T2, bumpShift)
	if h3, ok := intersectRayHyperFrustum(O3, T2, obj); ok && h3.t < 1e-6 {
		t.Fatalf("ray re-entered immediately after exit")
	}
}
