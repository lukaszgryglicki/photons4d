...
func nearestHit(scene *Scene, O Point4, D Vector4, tMax Real) (objectHit, bool) {
	...
	for _, s := range scene.Spherinders {
		...
		if hit, ok := intersectRaySpherinder(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// hypercones
	for _, s := range scene.HyperCones {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRayHyperCone(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// hypercapsules
	for _, s := range scene.HyperCapsules {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRayHyperCapsule(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// spheritori
	for _, s := range scene.Spheritori {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRaySpheritorus(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}
	...
	// duocylinders
	for _, s := range scene.Duocylinders {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRayDuocylinder(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// torispheres
	for _, s := range scene.Torispheres {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRayTorisphere(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// superquadrics
	for _, s := range scene.Superquadrics {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRaySuperquadric(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}

	// hyperfrustums
	for _, s := range scene.HyperFrustums {
		if ok, tNear := rayAABB(O, s.AABBMin, s.AABBMax, rr); !ok || tNear > bestT {
			continue
		}
		if hit, ok := intersectRayHyperFrustum(O, D, s); ok && hit.t > 1e-12 && hit.t < bestT {
			bestT, best, okAny = hit.t, hit, true
		}
	}
