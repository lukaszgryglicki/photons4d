...
func TestNewTorisphereAndIntersect(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NewTorisphere: %v", err)
	}

	// Through the tube cross-section centered at local (x=R, y=z=w=0), along +X.
	O := Point4{0, 0, 0, 0.6}
	D := Vector4{1, 0, 0, 0}
	h, ok := intersectRayTorisphere(O, D, obj)
	if !ok {
		t.Fatalf("expected torisphere hit")
	}
	if h.inv {
		t.Fatalf("expected entering hit")
	}
	if math.Abs(float64(h.t-0.17)) > 1e-9 {
	...
	O2 := Point4{0.25, 0, 0, 0.6}
	D2 := Vector4{1, 0, 0, 0}
	h2, ok2 := intersectRayTorisphere(O2, D2, obj)
	if !ok2 || !h2.inv {
...
func TestSceneAABBWithTorisphere(t *testing.T) {
	obj, err := NewTorisphere(
