...
func NewCell600(center Point4, radius Real, scale Vector4, angles Rot4, color, reflectivity, refractivity, ior RGB) (*Cell600, error) {
	if !(radius > 0) {
	...
	cp.buildPlanes(verts120Unit(), radius)

	DebugLog("Created 600-cell: center=%+v, radius=%.6g, scale=%+v, AABB=[%+v..%+v]", center, radius, scale, cp.AABBMin, cp.AABBMax)
