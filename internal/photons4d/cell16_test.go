...
func TestNewSixteenCellAndIntersect(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NewSixteenCell: %v", err)
	}

	// Simple axis ray along +W from origin
	O := Point4{0, 0, 0, 0}
	D := Vector4{0, 0, 0, 1}
	h, ok := intersectRaySixteen(O, D, c16)
	if !ok {
	...
	O2 := c16.Center
	D2 := Vector4{0, 0, 0, 1}
	h2, ok2 := intersectRaySixteen(O2, D2, c16)
	if !ok2 || !h2.inv {
		t.Fatalf("expected inside->exit hit; ok=%v inv=%v", ok2, h2.inv)
	}
}

func TestSceneAABBWithSixteenCell(t *testing.T) {
	c16, err := NewSixteenCell(
