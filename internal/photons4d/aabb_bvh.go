...
func collectSceneObjects(s *Scene) []bvhLeaf {
	out := make([]bvhLeaf, 0,
		len(s.Cells8)+len(s.Hyperspheres)+len(s.Cells5)+len(s.Cells16)+len(s.Cells24)+len(s.Cells120)+len(s.Cells600)+len(s.StarCells)+len(s.Spherinders)+len(s.HyperCones)+len(s.HyperCapsules)+len(s.Spheritori)+len(s.Duotori)+len(s.Duocylinders)+len(s.Torispheres)+len(s.Superquadrics)+len(s.HyperFrustums))
	...
	for _, o := range s.StarCells {
		...
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayStarCell(O, D, obj) },
		})
	}
	for _, o := range s.Spherinders {
		if o == nil {
			continue
		}
		obj := o
	...
	for _, o := range s.HyperCapsules {
		...
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayHyperCapsule(O, D, obj) },
		})
	}
	for _, o := range s.Spheritori {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRaySpheritorus(O, D, obj) },
		})
	}
	for _, o := range s.Duotori {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayDuotorus(O, D, obj) },
		})
	}
	for _, o := range s.Duocylinders {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayDuocylinder(O, D, obj) },
		})
	}
	for _, o := range s.Torispheres {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayTorisphere(O, D, obj) },
		})
	}
	for _, o := range s.Superquadrics {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRaySuperquadric(O, D, obj) },
		})
	}
	for _, o := range s.HyperFrustums {
		if o == nil {
			continue
		}
		obj := o
		out = append(out, bvhLeaf{
			min: obj.AABBMin, max: obj.AABBMax,
			intersect: func(O Point4, D Vector4) (objectHit, bool) { return intersectRayHyperFrustum(O, D, obj) },
		})
	}
	return out
}
